...
import {
  AIGeneratorCustomViewType,
  AIGeneratorData,
  AIGeneratorGridViewType,
  AI_GENERATOR_DATA,
  isCustomView,
...
const AIGeneratorInit = ({
  ...
  return (
    <div className="ai-generator-init">
      ...
      <span className="ai-generator-init-description">
        생성하고자 하는 카테고리를 선택해주세요.
      </span>
          ...
          return (
            <div className="ai-generator-init-category" onClick={() => onSelectCategory(category)}>
              ...
              <div className="ai-generator-init-category-info">
                <span className="ai-generator-init-category-name" style={{ color: category.color }}>
                  {category.name}
                </span>
                <span className="ai-generator-init-category-description">
...
const AIGeneratorMain = ({
  ...
  const viewRenderer = useMemo(() => {
    if (step === viewData[category.name].views.length + 1) {
      return <AIGeneratorDetailPrompt category={category} prompt={prompt} setPrompt={setPrompt} />;
    }
        ...
        <div className="ai-generator-main-header">
          <div className="ai-generator-main-header-title-wrapper">
            <div className="ai-generator-main-header-title">
              {step === viewData[category.name].views.length + 1 && <span>{'마지막 단계! '}</span>}
            ...
            {step === viewData[category.name].views.length + 1 && (
              <span className="ai-generator-main-header-description">
                {viewData[category.name].detailViewComment}
              </span>
            )}
          </div>
        </div>
        <div className="ai-generator-main-view">{viewRenderer}</div>
      </ScrollArea>
...
const AIGeneratorGridView = ({
  ...
  return (
    <div className="ai-generator-grid-view">
      ...
      <div
        className="ai-generator-grid-view-select"
        style={{ gridTemplateColumns: `repeat(${view.column}, 1fr)` }}
      >
        {view.data.map((data) => (
          <div
            className={cn('ai-generator-grid-view-element', {
              selected: prompt[view.key] === data.value,
            })}
            style={{
              backgroundColor: prompt[view.key] === data.value ? category.color : '#212121',
            }}
        ...
        data: [
          {
            ...
            value: 'LARGO',
            description:
              '차분하고 여유로운 리듬으로 감성적인 분위기를 연출합니다.\n감정 표현이 중요할 때 선택하세요.',
          },
          { label: '', value: 'ANDANTE' },
          {
            label: '중간',
          ...
          { label: '', value: 'ALLEGRETTO' },
          {
            ...
            value: 'ALLEGRO',
            description:
              '활기차고 에너지가 넘치는 곡을 만들고 싶을 때 선택하세요.\n빠른 속도로 강렬한 느낌을 줍니다.',
          },
        ],
      },
      {
        ...
        data: [
          {
            ...
            value: 'PIANISSIMO',
            description:
              '부드럽고 차분한 분위기를 조성하는 에너지입니다. 여유롭고 편안한 곡에 잘 어울립니다.',
          },
          { label: '', value: 'PIANO' },
          {
            label: '중간',
            value: 'MEZZO',
            description:
              '밸런스를 이루는 에너지로, 다양한 장르에 적합하며 적당한 힘과 활기를 더해줍니다.',
          },
          { label: '', value: 'FORTE' },
          {
            label: '강한',
            value: 'FORTISSIMO',
            description:
              '강렬하고 파워풀한 느낌을 주는 에너지입니다. 역동적이고 에너지가 넘치는 곡을 만드는데 이상적입니다.',
          },
        ],
      },
    ],
        ...
        <div className="ai-music-custom-wrapper">
          ...
          <div className="ai-music-custom-slider-wrapper">
            <div className="ai-music-custom-slider-labels">
              {data.data.map((el, index) => (
                <div className="ai-music-custom-slider-label-wrapper">
                  <span
                    className={cn('ai-music-custom-slider-label', {
                      selected: index === data.sliderValue,
                    })}
                  >
                    {el.label}
                  </span>
                  <span
                    className={cn('ai-music-custom-slider-label-dot', {
                      selected: index === data.sliderValue,
                    })}
                  />
                </div>
            ...
            <div className="ai-music-custom-description">
              {data.data[data.sliderValue].description && (
                <span>{data.data[data.sliderValue].description}</span>
              )}
            </div>
          </div>
        </div>
...
const MusicPromptSlider = ({
  ...
  return (
    <Slider
      sx={{ height: '26px !important', padding: '0 !important' }}
      value={value}
    ...
    <div className="ai-generator-visual-size-ratio">
      ...
      <div className="ai-generator-visual-size-ratio-view">
        <div className="ai-generator-visual-size-ratio-grid">
          {viewDate.map((el) => (
            <div
              className="ai-generator-visual-size-ratio-grid-item"
        ...
        <div className="ai-generator-visual-size-ratio-example">
          <div
            className="ai-generator-visual-size-ratio-example-box"
  ...
  return (
    <div className="ai-generator-detail-prompt">
      ...
      <textarea
        ...
        className="ai-generator-prompt-input"
      />
    </div>
  );
};
    ...
    return (
      <div className="ai-generator-processing">
        <div className="ai-generator-processing-loading">
          <div
            className="ai-generator-processing-icon"
            style={{ backgroundColor: `${category.color}4A` }}
          >
      ...
      <div className="music-result">
        ...
        <div className="music-progress-wrapper">
          ...
          <input
            ...
            disabled={!ready}
            style={{
              background: `linear-gradient(to right, #FF7193 ${playTime * 100}%, #554F4F 0)`,
            }}
