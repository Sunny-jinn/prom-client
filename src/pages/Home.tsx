...
import { Notification, getNotificationsAPI } from '@/feature/api/notification.api';
...
const Home = () => {
  ...
  const todayAlarm = useMemo(() => {
    return _.sortBy(
      alarms.filter((el) => dayjs().diff(el.createdAt, 'd') === 0),
      'createdAt',
    ).reverse();
  }, [alarms]);
      ...
      <Drawer isOpen={isOpen} onClose={onClose} placement={'right'} size={'full'}>
        ...
        <DrawerContent>
          <SafeAreaLayout flexDirection={'column'}>
            <div
              style={{ display: 'flex', padding: '0 16px', alignItems: 'center', width: '100%' }}
            >
        ...
        <NavigatorLayout hasScrollArea={true}>
          <div className="home-header">
            ...
            <div className="home-alarm">
              ...
              <Alarm
                onClick={() => (alarms.length > 0 ? onOpen() : undefined)}
                width={19}
                height={19}
              />
            </div>
