...
import { INIT_STEP_3, INIT_STEP_HEADER_TEXT, POST_CATEGORY_DATA } from '@/constants/init.data';
import {
  checkNicknameAPI,
  logoutAPI,
  updateUserInfoAPI,
  updateUserInterestAPI,
} from '@/feature/api/user.api';
      ...
      {step !== 3 && (
        <div id={'Init'}>
          ...
          <div className="init-header">
            <span className="init-header-title">{INIT_STEP_HEADER_TEXT[step].title}</span>
  ...
  return (
    <div className="init-step-1">
      ...
      <div className="init-button-wrapper">
        {!isAbleNickname && <span>이미 사용중인 닉네임입니다.</span>}
...
const InitStep2 = (props: InitStepProps) => {
  const { profile, setProfile, setStep } = props;
  const isAbleToNext = useIsAble([profile.interest.length > 0]);

  const isSelectedInterest = useCallback(
    (el: Interest) => {
    ...
    <div className="init-step-2">
      <div className="init-step-2-interest-list">
        {POST_CATEGORY_DATA.map((el) => {
          ...
          return (
            <div
              onClick={() => onSelectInterest(el.name as Interest)}
              style={{
                borderColor: isSelectedInterest(el.name as Interest) ? el.color : '#212121',
              }}
            ...
            >
              <div className="init-step-2-interest-content">
                ...
                <div className="init-step-2-interest-info">
                  <span className="init-step-2-interest-info-name" style={{ color: el.color }}>
                    {el.name}
                  </span>
              ...
              <Check
                fill={isSelectedInterest(el.name as Interest) ? el.color : '#212121'}
                width={19}
                height={14}
              />
            </div>
...
const InitStep3 = (props: InitStepProps) => {
  ...
  const isAbleToNext = useIsAble([
    profile.keywords !== null &&
      Object.values(profile.keywords).reduce((acc, cur) => cur.length >= 2 && acc, true),
  ]);
  const isSelected = useCallback(
    (name: Interest, keyword: string) => {
      if (!profile.keywords) {
        return false;
      }
