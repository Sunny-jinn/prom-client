...
const Upload = ({ onClose, isOpen }: UploadProps) => {
              ...
              <div className="upload-type">
                <div className="switch">
                  ...
                  <div className="slider">
                    <div
                      className={cn('type', { selected: uploadType === 'FEED' })}
                      onClick={() => setUploadType('FEED')}
                    >
                      <span>게시글</span>
                    </div>
                    <div
                      className={cn('type', { selected: uploadType === 'PICK' })}
...
const UploadHeader = (props: UploadHeaderProps) => {
  ...
  return (
    <div className="upload-header">
      <span style={{ color: '#ffffff' }} onClick={() => leftValue.onClick()}>
        {leftValue.text}
      </span>
      <span style={{ color: '#ffffff', fontWeight: 600 }}>{title}</span>
      <button
        disabled={rightValue.isAble}
        onClick={() => (rightValue.onClick ? rightValue.onClick() : undefined)}
      >
        {rightValue.text}
      </button>
    </div>
...
const UploadFeed = ({ onClose, uploadType, step, setStep }: UploadContentProps) => {
  ...
  const isAbleToNext = useIsAble([images.length !== 0, indexedImages.length !== 0]);
        ...
        <div className="upload-container">
          ...
          <div className="upload-content-selector-wrapper">
            <div className="upload-content-selector-header">
              <span>불러온 사진</span>
              {images.length !== 0 && (
                <span onClick={() => onClickUpload('GET')} style={{ color: '#B8B8B8' }}>
                  다시 불러오기
                </span>
              )}
            </div>
            ...
            <div className="upload-content-selector">
              {images.length === 0 && (
                <div className="upload-content-selector-empty" onClick={() => onClickUpload('GET')}>
                  ...
                  <span>
                    터치해 사진 불러오기 <span style={{ color: '#A6A6A6' }}>(최대 10장)</span>
                  </span>
                </div>
              )}
              {images.length !== 0 && (
                    ...
                    >
                      {images.map((el) => (
                        <SwiperSlide className={'feed-swiper-slide'}>
                          ...
                          <img
                            onClick={() => setSelectedPreview(el.url)}
                            src={el.url}
                            alt="preview"
                          />
                        </SwiperSlide>
...
const UploadPick = ({ onClose, uploadType, step, setStep }: UploadContentProps) => {
  ...
  const isAbleToNext = useIsAble([Boolean(pick)]);
        ...
        <div className="upload-container">
          ...
          <div className="upload-content-selector-wrapper">
            <div className="upload-content-selector-header">
              <span>불러온 PICK</span>
              {pick && (
                <span onClick={() => onClickUpload()} style={{ color: '#B8B8B8' }}>
                  다시 불러오기
                </span>
              )}
            </div>
...
const UploadFinalize = (props: UploadFinalizeProps) => {
  ...
  const isAble = useIsAble([title !== '', description !== '', type !== null]);
    ...
    <>
      <div className="upload-finalize">
        ...
        <ScrollArea style={{ padding: '0 16px' }}>
          <div className="upload-finalize-content">{children}</div>
          ...
          <div className="upload-finalize-type-wrapper">
            ...
            <div className="upload-finalize-type-list">
              {POST_CATEGORY_DATA.map((el) => {
                ...
                return (
                  <div
                    onClick={() => setType(el.name)}
                    className="upload-finalize-type"
                    style={{ borderColor: el.name === type ? el.color : '#000000' }}
                  >
                    <Icon height={14} width={14} />
                    <span style={{ fontSize: 14, color: el.color, fontWeight: 600 }}>
                      {el.name}
                    </span>
                  </div>
                );
  ...
  return (
    <div className="uploading">
      ...
      <div className="uploading-preview">
        <div
          className="uploading-preview-blur"
          style={{ borderRadius: type === 'PICK' ? '10px' : '15px' }}
        >
